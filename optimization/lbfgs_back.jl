...
#             iter_count += 1;
#
#             println("\nIteration: ", iter_count, ", before l-BFGS.")
...
#             matshow((reshape(vel_1,conf.Nx,conf.Ny))',cmap="PuBu"); colorbar(); title("$title_name Hz")
...
function l_BFGS(vel_init, conf, recorded_data, vmin, vmax; m=3, alpha_1=100, alpha_max=500, iter_time=5, c1=1e-11, c2=0.9, search_time=5, zoom_time=5, verbose=false, save_graph=false, fre_range="all")
	...
	for ind_fre = 1:conf.fre_num
		...
		for iter = m+2:1:iter_time
			...
			iter_count += 1;
			println("\nIteration: ", iter_count, ", frequency: ", conf.frequency[ind_fre], ", l-BFGS.")
