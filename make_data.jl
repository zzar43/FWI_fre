...
# Forward modelling
@time wavefield_true, recorded_data = scalar_helmholtz_solver(vel_true, conf; fre_range=[5], verbose=true);
...
matshow(real(wavefield_true[:,:,5,20])',cmap="RdBu",clim=[-0.2,0.2]); colorbar();
